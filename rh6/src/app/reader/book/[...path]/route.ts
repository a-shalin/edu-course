...
import { TEXTBOOK_ROOT_PATH } from "@/lib/course-config";

const ROOT_DIR = path.resolve(process.cwd(), TEXTBOOK_ROOT_PATH);
...
(function () {
  ...
  function scheduleHighlightAndNotify() {
  ...
  window.addEventListener("hashchange", function () {
    scheduleHighlightAndNotify();
  });

  window.addEventListener("load", function () {
    revealWhenFontsReady();
    scheduleHighlightAndNotify();
    window.setTimeout(highlightTarget, 120);
  });
