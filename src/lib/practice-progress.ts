...
export function getPracticeProgressCookieKey(chapterId: number): string {
  return `rh9_practice_progress_chapter_${chapterId}_v1`;
}
...
export function readSolvedPracticeItemIds(chapterId: number): string[] {
  ...
  try {
    ...
    if (
      parsed.v !== PRACTICE_PROGRESS_VERSION ||
      !Array.isArray(parsed.solvedItemIds)
    ) {
      return [];
    }

    return [...new Set(parsed.solvedItemIds.filter((value): value is string => typeof value === "string"))];
  } catch {
    return [];
  }
}
