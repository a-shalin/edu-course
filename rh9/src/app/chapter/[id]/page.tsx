...
function StudyCardView({
  ...
  return (
    <button
      type="button"
      data-testid={`study-card-${card.id}`}
      onClick={() => {
        if (target) {
...
function ChapterSummary({
  ...
  return (
    <details className="group mb-14 overflow-hidden rounded-[1.8rem] border border-border bg-card shadow-[0_20px_60px_rgba(59,37,26,0.08)]">
      <summary
        data-testid="chapter-summary-toggle"
        className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-5 marker:content-none lg:px-8"
      >
        <div>
          ...
          {entries.map((entry) => {
            ...
            return (
              <button
                key={entry.id}
                type="button"
                data-testid={`summary-entry-${entry.id}`}
                onClick={() => {
                  if (target) {
                    onOpenBookTarget(target);
                  }
        ...
        {chapter.status === "ready" ? (
          <>
            ...
            <section>
              ...
              <div data-testid="chapter-practice">
                <PracticeSession
